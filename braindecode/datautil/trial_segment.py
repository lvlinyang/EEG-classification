...
def _create_signal_target_from_cnt_y_start_stops(
    ...
    for i_start, i_stop in new_i_start_stops:
        ...
        # wavelet = Morlet(w0=6)
        #
        # batch = data[:, i_start:i_stop]
        #
        # ##############################################################
        # # Performing wavelet transform
        # wa_torch = WaveletTransformTorch(dt, dj, wavelet, unbias=unbias, cuda=True)
        #
        # power_torch = wa_torch.power(batch).astype(np.float32).transpose(1, 0, 2)
        #
        # batch_1 = np.expand_dims(batch, axis=0)
        #
        # c = np.r_[power_torch, batch_1]    # an lie jianjie liangge juzhen
        # ## c = np.reshape(c, (-1, 1125))
        # ## c = np.expand_dims(c,0)
        #
        #
        #
        # X.append(c.astype(np.float32))
        #
